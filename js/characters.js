...
// Helper function to get color for different coin types
...
function renderChars() {
  ...
  state.chars.forEach((c, ci) => {
    ...
    c.equipped.forEach((slotObj, si) => {
      const slot = createSlot(slotObj, ci, si, "equipped", c.equipped);
    ...
    function createSlot(slotObj, ci, si, section, slotsArray) {
...
// Add coin tracking if item is a Coin Purse
    ...
    c.backpack.forEach((slotObj, si) => {
      const slot = createSlot(slotObj, ci, si, "backpack", c.backpack);
    ...
    if (hasLargeSack) {
      ...
      c.largeSack.forEach((slotObj, si) => {
        const slot = createSlot(slotObj, ci, si, "largeSack", c.largeSack);
    ...
    // If no large sack but small sack is equipped
      ...
      c.smallSack.forEach((slotObj, si) => {
        const slot = createSlot(slotObj, ci, si, "smallSack", c.smallSack);
...
// Render the character list in the left panel
...
// Initialize character-related event listeners
